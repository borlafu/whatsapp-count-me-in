...
- **Auto-conclude**: Automatically concludes the event once its scheduled time is reached.
...
- `!cancel`: Deactivate the current active event (Admins only). A cancelled event never happened, so it does not count towards anyone's participation history.
- `!conclude`: Mark the active event as finished (Admins only). Timed events conclude by themselves once their scheduled time passes, so this is mainly for events created without a date. An event that has not reached its scheduled time yet cannot be concluded.
